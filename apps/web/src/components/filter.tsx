  ...
  return (
    ...
    >
          ...
          <div className="flex flex-col gap-2">
            {category?.map((v) => (
              <div key={v.id}>
                <CCheck
                label={v.name}
                value={selectedCategoryId?.some((id) => id === v.id) || false}
                onChange={(check) => {
                  if (check) {
                    const newCategory = [...(selectedCategoryId || []), v.id];
                    if (onChangeCategory) {
      ...
      <AccordionItem className="border-b" value="item-4">
        ...
        <AccordionContent>
          <div className="flex flex-row gap-2 items-center">
            <Input placeholder="Minimum Order" className="focus:ring-inset" />
      ...
      <AccordionItem className="border-b" value="item-5">
        ...
        <AccordionContent>
          <div className="flex flex-row gap-2 items-center">
            <Input placeholder="Minimum Stock" className="focus:ring-inset" />
            <Button variant="default" size="default">
              Ok
            </Button>
          </div>
